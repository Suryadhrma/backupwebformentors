...
const TopNavigationMentor = ({ currentPage }) => {
    const navigate = useNavigate(); // Correct the usage here with parentheses
    const [searchTerm, setSearchTerm] = useState('');
    const [balance, setBalance] = useState(null);

    useEffect(() => {
        // Simulating a fetch call for balance data
        setTimeout(() => {
            setBalance(125.75);
        }, 1000);
    }, []);
    ...
    const handleCourseClick = () => {
        navigate('/mentor/create-course'); // Correct function call
    }

    const handleAdsClick = () => {
        navigate('/mentor/create-ads'); // Correct function call
    }
